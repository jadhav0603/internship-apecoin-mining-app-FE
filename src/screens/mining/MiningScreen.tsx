...
const MiningScreen = () => {
  ...
  useEffect(() => {
    refreshMiningStatusRef.current = refreshMiningStatus;
  }, [refreshMiningStatus]);

  useFocusEffect(
    useCallback(() => {
      void refreshMiningStatusRef.current().catch(error => {
        if (__DEV__) {
          console.warn('[mining] focus refresh failed', error?.message);
        }
      });
    }, []),
  );
